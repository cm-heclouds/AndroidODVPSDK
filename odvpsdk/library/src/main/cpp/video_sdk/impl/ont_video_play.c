...
int32_t ont_vod_start_notify(ont_device_t *dev, const char* location,int32_t channel, uint8_t protype, const char *playflag, const char *pushurl, uint16_t ttl)
{
	VARAIBLE_CHECK;
	t_playlist data2 = { {0} };
	ont_list_node_t *node;
	...
	ont_platform_snprintf(data2.playflag, sizeof(data2.playflag), "%d_%s", channel, playflag);
	...
	do
	{
		...
		if (rtmp_rvod_mp4_parse(mp4ctx, location) < 0)
		{
			break;
		}
		...
		ont_platform_snprintf(data->playflag, sizeof data->playflag, "%d_%s", channel, playflag);
		data->playctx = ctx;
