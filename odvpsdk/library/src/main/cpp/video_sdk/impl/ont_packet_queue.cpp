        ...
        unsigned int deltaTs, int codecType, void *data, RTMPVideoAudioCtl *ctl)
{
    ...
    if (pkdataHead && playctx->last_sndts - pkdataHead->sndts > 5000) {
        playctx->state = 1;
		RTMP_Log(RTMP_LOGINFO, "live packet enqueue, last sndts %lu, buffer header sndts %lu", playctx->last_sndts, pkdataHead->sndts);
        return -4;
    }
#endif
	//RTMP_Log(RTMP_LOGINFO, "type %d, ts %d, size %d", codecType, deltaTs, size);
    ...
    if (CODEC_H264 == codecType && data) {
		...
		//RTMP_LogHexString(RTMP_LOGINFO, pFrameBuf, size);
...
int RTMPRvodPackDequeue(void *ctx)
{
    ...
    case CODEC_MPEG4A:
        if (playctx->sendmeta[1] == 0) {
			...
			rtmp_send_audiodata(vodctx->rtmp, playctx->audioHeaderTag, audio_data, 3, pkdata->deltaTs, RTMP_PACKET_SIZE_LARGE);
...
int RTMPLivePackDequeue(void *ctx)
{
	...
	case CODEC_H264:
        ...
        if (rtmp_send_videodata(playctx->rtmp_client, (unsigned char *)pkdata->data, pkdata->size, 
            ...
            playctx->state = 1;
            RTMP_Log(RTMP_LOGINFO, "live packet dequeue, send video data, state is %d", playctx->state);
            goto _end;
        } 
        break;
    case CODEC_MPEG4A:
        if (playctx->sendmeta[1] == 0) {
			...
			unsigned char audio_data[3];
			audio_data[0] = 0x00;
			audio_data[1] = playctx->audio_seq[0];
			audio_data[2] = playctx->audio_seq[1];

            if (rtmp_send_audiodata(playctx->rtmp_client, pkdata->ahd->audioTag, audio_data, 3,
