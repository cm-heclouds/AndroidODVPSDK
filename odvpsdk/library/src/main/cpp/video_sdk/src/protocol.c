...
static int ont_device_msg_channel_record_callback(ont_device_t *dev, char *msg, size_t length)
{
	uint32_t channel;
	uint8_t status;
	uint32_t duration;
	...
	status = buffer[index++];
	duration = ont_decodeInt32(buffer + index);
	index += 4;
	pushlen = ont_decodeInt16(buffer + index);
	index += 2;
	...
	memcpy(push_url, buffer + index, pushlen);
	push_url[pushlen] = '\0';
    if (!dev->device_cbs->record_status)
    {
        return ONT_ERR_INTERNAL;
    }
	return dev->device_cbs->record_status(dev, channel, status, duration, push_url);
}
...
static int ont_device_msg_userdefined_cmd_callback(ont_device_t *dev, char *msg, size_t length)
{
    ...
    memset(&cmd, 0x00, sizeof(ont_device_cmd_t));
    ...
    if (!dev->device_cbs->user_defined_cmd)
    {
        return ONT_ERR_INTERNAL;
    }
...
static int ont_device_msg_apidefined_activated_msg_callback(ont_device_t *dev, char *msg, size_t msg_len)
{
	if (NULL == dev || NULL == msg)
	{
		return ONT_ERR_BADPARAM;
	}
    if (!dev->device_cbs->api_defined_msg)
    {
        return ONT_ERR_INTERNAL;
    }
	return dev->device_cbs->api_defined_msg(dev, msg, msg_len);
}
...
int ont_device_msg_callback(ont_device_t *dev, char *msg, size_t length, t_message_header *header)
{
	...
	switch (header->msg_type)
	{
		...
		case MSG_TYPE_USERDEFINED_ACTIVATED_CMD:
		{
			ret = ont_device_msg_apidefined_activated_msg_callback(dev, msg, length - ONT_PROTOCOL_COMM_HEADER_LENGTH);
			break;
		}
		case MSG_TYPE_CHANNEL_RECORD_STATUS:
		{
			ret = ont_device_msg_channel_record_callback(dev, msg, length);
			break;
		}
		default:
		{
	...
	if (header->msg_type==MSG_TYPE_LIVE_START||
		...
		header->msg_type == MSG_TYPE_CHANNEL_RECORD_STATUS)
	{
		r_byte = (char)ret;
...
int ont_device_msg_register(ont_device_t *dev, t_message_register *reg)
{
	...
	*pData++ = reg->deviceid_size;
	...
	if (ret == ONT_ERR_OK)
	{
		...
		{
			ret = result;
			RTMP_Log(RTMP_LOGERROR, "register ret is %d", result);
		}
	}
...
int ont_device_msg_addchannel(ont_device_t *dev, t_message_addchannel *chn)
{
	...
	*pData++ = chn->desc_length;
	...
	if (ret == ONT_ERR_OK)
	{
		...
		{
            ret = result;
			RTMP_Log(RTMP_LOGERROR, "add channel ret is %d", result);
		}
	}
...
int ont_device_msg_upload_picture(ont_device_t *dev, t_message_picture *pic)
{
    ...
    char *data = ont_platform_malloc(sizeof(t_message_picture) + pic->name_length + pic->picture_length);
	char *pData = data;    

    ont_encodeInt32(pData, pic->chnid);
    pData += 4;
    
   *pData++ = pic->picture_type;

    ont_encodeInt16(pData, pic->name_length);
    pData += 2;

    memcpy(pData, pic->name, pic->name_length);
	pData += pic->name_length;    
     
    ont_encodeInt24(pData, pic->picture_length);
    pData +=3;

    memcpy(pData, pic->picture, pic->picture_length);
    pData += pic->picture_length;   
    ...
    ret = ont_device_msg_sendmsg(dev, MSG_TYPE_UPLOAD_PICTURE, data, pData - data, 1, resp, &resp_len, 5000);
	...
	if (ret == ONT_ERR_OK)
	{
		...
		{
            ret = result;
			RTMP_Log(RTMP_LOGERROR, "upload picture ret is %d", result);
		}
	}
...
int ont_device_msg_upload_preview_picture(ont_device_t *dev, t_message_pre_picture *pic)
{
    ...
    /* 8为odvp 0x0012预览图片协议前3个字段的大小 */
    data = ont_platform_malloc(8 + pic->size);
    if (NULL == data) {
   ...
   *pData = pic->type;
   pData  += 1;
    ...
    ont_encodeInt24(pData, pic->size);
    pData += 3;

    memcpy(pData, pic->pic, pic->size);
	pData += pic->size;     

    ret = ont_device_msg_sendmsg(dev, MSG_TYPE_UPLOAD_PIC_PREVIEW, data, pData - data, 1, resp, &resp_len, 5000);
	if (ret == ONT_ERR_OK)
	{
		result = resp[0];
		if (result)
		{
            ret = result;
			RTMP_Log(RTMP_LOGERROR, "upload preview picture error, ret is %d", result);
		}
	}
